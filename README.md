...
```bash
aws cloudformation delete-stack --stack-name symbol-api
```
...
sam deploy --config-env testnet --profile sharpshark
sam deploy --config-env mainnet --profile sharpshark

sam local invoke ApostilleFunction -e events/default.json --env-vars env-prod.json --profile sharpshark